<script setup>
    ...
    const openPdfPreview = (filePath) => {
        selectedFile.value = `/storage/${filePath}`;
        isModalPreviewActive.value = true;
    }

    const closePdfPreview = () => {
        isModalPreviewActive.value = false;
        selectedFile.value = '';
    }
</script>
        ...
        <table class="w-full my-2 text-left text-gray-800 rtl:text-right">
            ...
            <tbody class="font-medium text-gray-600">
                <tr v-for="document in itemsPaginated" :key="document.id"
                    ...
                    <td data-label="Title" scope="row" class="px-6 py-6">
                        {{ document . title }}
                    </td>
                    <td data-label="Due on" class="px-4 py-1 text-red-800 w-1/6">
                    ...
                    <td v-if="file" class="py-1 pr-8 text-center w-1/3">
                        <BaseButtons type="justify-end" no-wrap>
                            <!-- Status for Completed Documents -->
                            <div v-if="isCompleted(document)"
                                class="flex items-center justify-center px-4 h-full font-semibold text-green-500">
                            ...
                            <BaseButton v-if="isSubmitted(document)" roundedFull color="blue" :icon="mdiEye"
                            ...
                            <BaseButton v-if="!isCompleted(document)"
                                ...
                                :color="visibleInputs[document.id] ? 'danger' : 'info'" small roundedFull
                                @click="toggleFileInput(document.id)" class="py-1 mr-1" />

                            <!-- File Input -->
                            <input v-if="visibleInputs[document.id] && !isCompleted(document)" type="file"
                                @change="event => handleFileSelect(document, event)"
                                class="p-1 mb-2 border rounded-md w-36" />

                            <!-- Submit Button -->
                            <BaseButton v-if="visibleInputs[document.id] && !isCompleted(document)"
                                :label="buttonLabel(document)" roundedFull color="success" small class="ml-2"
                                @click="submitFile(document)" :disabled="isCompleted(document)" />
                        </BaseButtons>
                    </td>
